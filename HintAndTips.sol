...
contract Tips {
    /**
    1. 28% смарт-контрактов никак не обрабатывают send(), возвращающий boolean.
    ...
    /**
    2. Вызов constant функций контракта, которые никак не изменяют состояния контракта, по сути - getter'ы,
    ...
    /**
    3. Все функции, у которых отсутствуют модификаторы pure или view, считаются компилятором функциями,
    изменяющими стейт, даже если на самом деле это чистые функции.
    Из-за этого будет сжигаться газ, повышая стоимость работы со смарт-контрактом 
     */
}